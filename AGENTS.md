...
- Full functional test suite (Hurl), default `dev` env:
  `make test` (== `hurl tests/hurl/api.hurl --variables-file ./hurl/env/dev.env --test`)
- Target a different env file: `make ENV=prod test`
...
- `docker-compose.yml` and `k8s/*.yaml` use different DB passwords: compose uses `password`,
  k8s Secret uses `postgres`. Don't assume they match.
- `k8s/api-deployment.yaml` ships with a literal `DOCKER_USERNAME` placeholder — README replaces
  it via `sed` before `kubectl apply`. The image is pulled from Docker Hub at deploy time.
